...
namespace Cinemachine
{
    ...
    public class CinemachineFreeLook : CinemachineVirtualCameraBase
    {
        ...
        public CinemachineOrbitalTransposer.Recentering m_RecenterToTargetHeading = new CinemachineOrbitalTransposer.Recentering(false, 1, 2);
        /// <summary>The coordinate space to use when interpreting the offset from the target</summary>
        [Header("Orbits")]
        [Tooltip("The coordinate space to use when interpreting the offset from the target.  This is also used to set the camera's Up vector, which will be maintained when aiming the camera.")]
        public CinemachineOrbitalTransposer.BindingMode m_BindingMode = CinemachineOrbitalTransposer.BindingMode.SimpleFollowWithWorldUp;
        /// <summary></summary>
        ...
        override public CameraState State
        {
            get
            {
                return m_State;
        ...
        private CinemachineVirtualCamera[] CreateRigs(CinemachineVirtualCamera[] copyFrom)
        {
            ...
            mOrbitals = null;
            float[] softCenterDefaultsV = new float[]
            {
                0.5f, 0.55f, 0.6f
            };
        ...
        private void UpdateRigCache()
        {
            ...
            foreach (var rig in m_Rigs)
            {
                ...
                rig.m_LockStageInInspector = new CinemachineCore.Stage[]
                {
                    ...
                    for (int i = 0; i < rigNames.Length; ++i)
                    {
                        if (mOrbitals[i] == null && go.name == rigNames[i])
                        {
                            ...
                            if (mOrbitals[i] != null)
                            {
                                ...
                                if (i == 0)
                                    mOrbitals[i].HeadingUpdater = (CinemachineOrbitalTransposer orbital, float deltaTime, Vector3 up)=>
                                    {
        ...
        void PushSettingsToRigs()
        {
            ...
            for (int i = 0; i < m_Rigs.Length; ++i)
            {
                ...
                if (CinemachineCore.sShowHiddenObjects)
                    m_Rigs[i].gameObject.hideFlags &= ~(HideFlags.HideInHierarchy | HideFlags.HideInInspector);
                else
                    m_Rigs[i].gameObject.hideFlags |= (HideFlags.HideInHierarchy | HideFlags.HideInInspector);
                //UnityEngine.Profiling.Profiler.EndSample();
